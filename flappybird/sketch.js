...
function preload() {
    ...
    ending = loadSound('sounds/firework.wav')
    hurt = loadSound('sounds/hurt.wav')
    music = loadSound('sounds/marios_way.mp3')
    bonus = loadSound('sounds/children.mp3')
...
function title() {
    ...
    textSize(28)
    textAlign(CENTER)
    text("Flappy Swann-Pye", width / (2 * 1.74), 100)
    textSize(14)
...
function done() {
    ...
    if (points > 0) {
        text("YESSSS", width / (2 * 1.74), 100)
    }
    else {
        text("Loser!!!", width / (2 * 1.74), 100)
    }
    textSize(14)
